...
def get_trait_summary(trait, info, vcf_obj=None, sample=None):
    ...
    This version safely handles missing SNPs by guarding against None returns from get_genotype.
    ...
    # Lactose Intolerance (MCM6 rs4988235)
        ...
        if gt is None:
            return "Unknown"
        return "Lactose tolerant" if 1 in gt else "Lactose intolerant"
...
# Red-Green Colourblindness (OPN1LW/OPN1MW proxies)
    ...
    # Dabigatran activation (CES1 variants)
        ...
        if gt is None:
            return "Unknown"
        return "Altered dabigatran activation" if 1 in gt else "Normal dabigatran activation"
    ...
    # Atomoxetine response (CYP2D6 variants)
        ...
        if gt is None:
            return "Unknown"
        return "Altered atomoxetine clearance" if 1 in gt else "Normal atomoxetine clearance"
    ...
    # Carbamazepine hypersensitivity (HLA-B; HLA-A)
    elif trait == "Carbamazepine hypersensitivity":
        gt = safe_gt(info["snps"][0])
        if gt is None:
            return "Unknown"
        return "High risk of carbamazepine hypersensitivity" if 1 in gt else "Low risk of carbamazepine hypersensitivity"
    ...
    for trait in selected:
        ...
        # Red-Green Colourblindness
            ...
            if gt is None:
                ...
                summary = "Red green colour blind" if any(gt) else "Not red green colour blind"
        ...
        # Fallback
            ...
            summary = ""
         # Store the summary centrally
        traits_info[trait]["summary"] = summary
...
for trait in selected:
    ...
    # Case 1: Externally Visible Characteristics → render flat
    if trait in trait_groups.get("Externally Visible Characteristics", []):
        st.subheader(trait)
            ...
            for snp in info["snps"]:
                if page == "Individual":
                    ...
                    if m_result is None:
                        ...
                        m_b, m_a = "NA/NA", "Not found in VCF"
                        m_zg = "Unknown"
                        ...
                        if m_gt is None:
                            m_b, m_a = "NA/NA", "No genotype call"
                            m_zg = "Unknown"
                            m_pres, m_mode = "Unknown", ""
                        else:
                    ...
                    if f_result is None:
                        ...
                        f_b, f_a = "NA/NA", "Not found in VCF"
                        f_zg = "Unknown"
                        ...
                        if f_gt is None:
                            f_b, f_a = "NA/NA", "No genotype call"
                            f_zg = "Unknown"
                            f_pres, f_mode = "Unknown", ""
                        else:
    ...
    # Case 2: All other traits → render inside an expander
    else:
