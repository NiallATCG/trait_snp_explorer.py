...
def get_trait_summary(trait, info):
    ...
    # PTC Tasting
        ...
        alt_count = sum(get_genotype(s, "ind")[0].count(1) for s in info["snps"])
        return "Can taste PTC" if alt_count > 0 else "Cannot taste PTC"

    # Coriander Taste
    elif trait == "Coriander Taste":
        gt = get_genotype(info["snps"][0], "ind")[0]
        return "Perceives coriander as soapy" if any(gt) else "Normal coriander taste"

    # Red-Green Colourblindness
    elif trait == "Red-Green Colourblindness":
        gt = get_genotype(info["snps"][0], "ind")[0]
        return "Red-green colour blind" if any(gt) else "Not red-green colour blind"

    # Sprint Gene
    elif trait == "Sprint Gene":
        gt = sorted(get_genotype(info["snps"][0], "ind")[0])
        return "Sprint gene present: better sprint performance" if 0 in gt else "Sprint gene absent: reduced sprint"

    # Alcohol Flush
    elif trait == "Alcohol Flush":
        gt = get_genotype(info["snps"][0], "ind")[0]
        return "Alcohol flush present" if any(gt) else "Alcohol flush not present"

    # Dermatology traits
    elif trait in ["Tanning Response","Lentigines (Sun Spots)","Wrinkle & Collagen Degradation","Stretch Marks (Striae Distensae)"]:
        has_alt = any(1 in get_genotype(s, "ind")[0] for s in info["snps"])
        if trait == "Tanning Response":
            return "Poor tanning / burns easily" if has_alt else "Good tanning ability"
        elif trait == "Lentigines (Sun Spots)":
            return "Higher likelihood of sun spots" if has_alt else "Lower likelihood of sun spots"
        elif trait == "Wrinkle & Collagen Degradation":
            return "Higher wrinkle susceptibility" if has_alt else "Lower wrinkle susceptibility"
        elif trait == "Stretch Marks (Striae Distensae)":
            return "Higher stretch mark susceptibility" if has_alt else "Lower stretch mark susceptibility"

    # Pharmacogenetic traits (Warfarin, Statins, Clopidogrel, etc.)
        ...
        return ", ".join(parts) if parts else "Typical response"

    elif trait == "Statin myopathy risk":
        slco1b1 = 1 in get_genotype("rs4149056", "ind")[0]
        abcg2 = 1 in get_genotype("rs2231142", "ind")[0]
        return "Elevated myopathy risk" if (slco1b1 or abcg2) else "Typical risk"
        ...
        if lof and not gain: return "Reduced activation (LOF)"
        elif gain and not lof: return "Higher activity (*17)"
        elif lof and gain: return "Mixed metabolism (LOF + *17)"
        else: return "Typical activation"

    elif trait == "Dabigatran activation":
        ces1 = 1 in get_genotype("CES1 variants", "ind")[0]
        return "Altered activation (CES1)" if ces1 else "Typical activation"
        ...
        return "Altered opioid metabolism" if (d6_loss or cnv) else "Typical metabolism"

    elif trait == "Atomoxetine response":
        d6var = 1 in get_genotype("CYP2D6 variants", "ind")[0]
        return "Higher atomoxetine exposure" if d6var else "Typical exposure"

    elif trait == "Tricyclic antidepressant response":
        d6var = 1 in get_genotype("CYP2D6 variants", "ind")[0]
        c19var = 1 in get_genotype("CYP2C19 variants", "ind")[0]
        return "Genotype-guided dosing advised" if (d6var or c19var) else "Typical dosing"

    elif trait == "SSRI response":
        d6var = 1 in get_genotype("CYP2D6 variants", "ind")[0]
        c19var = 1 in get_genotype("CYP2C19 variants", "ind")[0]
        return "Genotype impacts exposure/tolerability" if (d6var or c19var) else "Typical exposure"
        ...
        return "Elevated SJS/TEN risk" if (b1502 or a3101) else "Typical risk"

    elif trait == "Phenytoin toxicity risk":
        cyp2c9 = any(1 in get_genotype(s, "ind")[0] for s in ["CYP2C9*2","CYP2C9*3"])
        b1502 = 1 in get_genotype("HLA-B*15:02", "ind")[0]
        return "Higher toxicity risk" if (cyp2c9 or b1502) else "Typical risk"

    elif trait == "Valproic acid and POLG":
        polg = 1 in get_genotype("POLG mutations", "ind")[0]
        return "Avoid valproate (POLG risk)" if polg else "Typical suitability"

    elif trait == "Siponimod contraindication":
        poor_cyp2c9 = any(1 in get_genotype(s, "ind")[0] for s in ["CYP2C9*2","CYP2C9*3"])
        return "Genotype-guided use; contraindications possible" if poor_cyp2c9 else "Typical suitability"
        ...
        return "High toxicity risk (DPYD)" if dpyd_flags else "Typical risk"

    elif trait == "Irinotecan toxicity risk":
        ugt = 1 in get_genotype("UGT1A1*28", "ind")[0]
        return "Higher neutropenia risk (UGT1A1*28)" if ugt else "Typical risk"

    elif trait == "Tamoxifen efficacy":
        d6var = 1 in get_genotype("CYP2D6 variants", "ind")[0]
        return "Potentially reduced efficacy (CYP2D6)" if d6var else "Typical efficacy"
        ...
        return "Severe myelosuppression risk" if (tpmt or nudt15) else "Typical risk"
        ...
        return "Higher cardiotoxicity risk" if (rarg or slc28a3) else "Typical risk"

    elif trait == "Abacavir hypersensitivity":
        b5701 = 1 in get_genotype("HLA-B*57:01", "ind")[0]
        return "Contraindicated (HLA-B*57:01)" if b5701 else "Eligible"

    elif trait == "Allopurinol severe skin reaction risk":
        b5801 = 1 in get_genotype("HLA-B*58:01", "ind")[0]
        return "High SCAR risk (HLA-B*58:01)" if b5801 else "Typical risk"

    elif trait == "Flucloxacillin liver injury risk":
        b5701 = 1 in get_genotype("HLA-B*57:01", "ind")[0]
        return "Higher DILI risk (HLA-B*57:01)" if b5701 else "Typical risk"

    elif trait == "Efavirenz exposure":
        c2b6 = 1 in get_genotype("rs3745274", "ind")[0]
        return "Higher exposure; dose reduce" if c2b6 else "Typical exposure"

    elif trait == "Atazanavir hyperbilirubinaemia":
        ugt = 1 in get_genotype("UGT1A1*28", "ind")[0]
        return "Benign hyperbilirubinaemia likely" if ugt else "Typical likelihood"

    elif trait == "Voriconazole dosing":
        cyp = any(1 in get_genotype(s, "ind")[0] for s in ["CYP2C19*2", "CYP2C19*3", "CYP2C19*17"])
        return "Genotype-guided dosing advised" if cyp else "Typical dosing"
        ...
        return "Potential impact; evidence emerging" if impdh else "Typical response"
        ...
        return "Tailor therapy (nicotine dependence/clearance)" if (chrna5 or cyp2a6) else "Typical response"

    elif trait == "Bupropion dosing":
        c2b6 = 1 in get_genotype("rs3745274", "ind")[0]
        return "Dose adjust (CYP2B6)" if c2b6 else "Typical dosing"
